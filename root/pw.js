...
function generatePW() {
    ...
    var hanzi = document.getElementById('hanziChecked')
        // build source string
    if (latin.checked) {
        base = base + latinLetters;
    }
    if (cyrillic.checked) {
        base = base + cyrillicLetters;
    }
    if (greek.checked) {
        base = base + greekLetters;
    }
    if (numbers.checked) {
        base = base + numbersLetters + numbersLetters + numbersLetters;
    }
    if (signs.checked) {
        base = base + signsLetters + signsLetters;
    }
    if (armenian.checked) {
        base = base + armenianLetters;
    }
    if (hangul.checked) {
        base = base + hangulLetters;
    }
    if (nordic.checked) {
        base = base + nordicLetters;
    }
    if (arabic.checked) {
        base = base + arabicLetters;
    }
    if (georgian.checked) {
        base = base + georgianLetters;
    }
    if (ethiopian.checked) {
        base = base + ethiopianLetters;
    }
    if (thaana.checked) {
        base = base + thaanaLetters;
    }
    if (hanzi.checked) {
        base = base + hanziLetters;
    }
    ...
    if (cyrillic.checked) {
        result.replaceAt((Math.random() * result.length), cyrillicLetters.charAt(Math.random() * cyrillicLetters.length));
    }
    if (greek.checked) {
        result.replaceAt((Math.random() * result.length), greekLetters.charAt(Math.random() * greekLetters.length));
    }
    if (armenian.checked) {
        result.replaceAt((Math.random() * result.length), armenianLetters.charAt(Math.random() * armenianLetters.length));
    }
    if (hangul.checked) {
        result.replaceAt((Math.random() * result.length), hangulLetters.charAt(Math.random() * hangulLetters.length));
    }
    if (nordic.checked) {
        result.replaceAt((Math.random() * result.length), nordicLetters.charAt(Math.random() * nordicLetters.length));
    }
    if (arabic.checked) {
        result.replaceAt((Math.random() * result.length), arabicLetters.charAt(Math.random() * arabicLetters.length));
    }
    if (georgian.checked) {
        result.replaceAt((Math.random() * result.length), georgianLetters.charAt(Math.random() * georgianLetters.length));
    }
    if (ethiopian.checked) {
        result.replaceAt((Math.random() * result.length), ethiopianLetters.charAt(Math.random() * ethiopianLetters.length));
    }
    if (thaana.checked) {
        result.replaceAt((Math.random() * result.length), thaanaLetters.charAt(Math.random() * thaanaLetters.length));
    }
    if (hanzi.checked) {
        result.replaceAt((Math.random() * result.length), hanziLetters.charAt(Math.random() * hanziLetters.length));
    }
    if (latin.checked) {
